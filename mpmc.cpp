...
/*
 * The main idea is to turn MPMS into effectively a SPSC, by generating monotonically
 * a read / write slot for an asking reader/writer, and then reader/writer has exlusive
 * access to the assign slot.
 */
...
class MPMC {
    ...
    // use i & mask for actual index into the ring buffer
...
public:
    MPMC() {
        for(int i = 0; i < N; ++i) {
            ...
            sequence[i].store(i, std::memory_order_release);
        }
    }
    void push(T&& item) {
